...
describe("userProfile service", () => {
  ...
  describe("loadUserProfile", () => {
    it("returns null for empty sheet", async () => {
      const result = await loadUserProfile(createMockDoc(createMockSheet([])));
    ...
    it("loads existing profile", async () => {
      ...
      const result = await loadUserProfile(createMockDoc(createMockSheet([row])));
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
  ...
  describe("saveUserProfile", () => {
    it("creates a row in an empty sheet", async () => {
      ...
      const result = await saveUserProfile(profile, createMockDoc(mockSheet));
      expect(result.isOk()).toBe(true);
    ...
    it("updates existing row", async () => {
      ...
      const result = await saveUserProfile({ age: 25 }, createMockDoc(mockSheet));
      expect(result.isOk()).toBe(true);
  ...
  describe("migrateFromLocalStorage", () => {
    ...
    it("migrates profile + lifecycle out of localStorage", async () => {
      ...
      const mockSheet = createMockSheet([]);
      const result = await migrateFromLocalStorage(createMockDoc(mockSheet));
